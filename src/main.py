...
class DataProcessorBase:
    ...
    def plot_data(self, x_axis_range, normalized_ideal, normalized_train, best_fit_indices):
        for i in range(4): 
            plt.title(f'F{best_fit_indices[i]}')
            plt.plot(x_axis_range, normalized_ideal.iloc[:, best_fit_indices[i]], label='IDEAL',color='red',)
            plt.plot(x_axis_range, normalized_train.iloc[:, i + 1], label='TRAIN',color='blue',linestyle='-')
            plt.legend()
            plt.show()
